...
{
    n--;
    n |= n >> 1;   // Divide by 2^k for consecutive doubles of k up to 32
    n |= n >> 2;
    n |= n >> 4;
    n |= n >> 8;
    n |= n >> 16;
...
void parallel_strassen_helper(matrix* m1, matrix* m2, matrix* res, unsigned int depth, unsigned int maxDepth, unsigned int naiveSize)
{
    if (depth>=maxDepth || m1->rowNum==naiveSize || m1->rowNum==2)
    {
        naive_mult(m1, m2, res);
...
void strassen_mult(matrix* orig1, matrix* orig2, matrix* res, unsigned int maxDepth, unsigned int naiveSize)
{
    ...
    else
    {
        ...
        parallel_strassen_helper(m1, m2, r, 0, maxDepth, naiveSize);
